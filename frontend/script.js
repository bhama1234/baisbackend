...
function appendMessage(sender, message) {
  ...
  messageElem.className = `animate-fade ${sender === "user" ? "text-right" : "text-left"}`;
  messageElem.innerHTML = `
    <span class="block text-sm ${sender === "user" ? "text-blue-300" : "text-green-400"} font-semibold">
      ${sender === "user" ? "🧑‍💼 You" : "🤖 BAIS"}
    </span>
    <div class="bg-gray-800 rounded-lg px-4 py-2 mt-1 inline-block max-w-[80%] ${sender === "user" ? "ml-auto" : "mr-auto"}">
      ${message}
    </div>
  `;
